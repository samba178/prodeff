...
*/
		...
		// Remove this check in case WooCommerce invents a new object in future
// 		if (!is_array($taxes)) return false;
		...
		$currency = method_exists($order, 'get_currency') ? $order->get_currency() : (method_exists($order, 'get_order_currency') ? $order->get_order_currency() : $base_currency);
		...
		// N.B. In WC 2.7+, what is returned is an array of WC_Order_Item_Tax objects; that class implements an array-access interface
...
// &tab=tax
			...
			$settings_link = '<a href="'.admin_url('admin.php').'?page='.$page.'">'.__("EU VAT Compliance Dashboard", "woocommerce-eu-vat-compliance").'</a>';
	...
	public function plugins_loaded() {
		...
		$this->settings = array(array(
			...
			'desc' 		=> __( "If this option is selected, then <strong>all</strong> orders by EU customers (whether consumer or business) which contain goods subject to variable EU VAT (whether the customer is exempt or not) will be forbidden.", 'woocommerce-eu-vat-compliance').' ',
...
// 			$this->settings[] = array(
// 				'name' 		=> __( "Show prices based on visitor's GeoIP-detected country", 'woocommerce-eu-vat-compliance' ),
// 				'desc' 		=> __( "If this option is selected, then tax calculations will take into account the visitor's apparent country, without them needing to select a country at the cart of checkout.", 'woocommerce-eu-vat-compliance' ),
...
// 		}
	...
	public function admin_notice_no_geoip_database() {
		...
		echo '<h4 style="margin: 1em 0 0 0">'.__('GeoIP database not found', 'woocommerce-eu-vat-compliance').'</h4><p>';
		echo __('You have the GeoIP plugin installed, but it has not yet downloaded its database. This is needed for country detection to work.', 'woocommerce-eu-vat-compliance');
	...
	public function admin_notice_no_geoip_plugin() {
		echo '<div class="error">';
		echo '<h4 style="margin: 1em 0 0 0">'.__('Required Plugin Not Found', 'woocommerce-eu-vat-compliance').'</h4><p>';
		echo __('For the WooCommerce EU VAT compliance module to be able to record the country that a customer is ordering from, the (free) GeoIP Detection plugin must be installed and activated.', 'woocommerce-eu-vat-compliance').' ';

		if (current_user_can('install_plugins')) {
			if (!file_exists(WP_PLUGIN_DIR.'/geoip-detect')) {
// 				echo '<a href="'.wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=geoip-detect'), 'install-plugin_geoip-detect').'">'.__('Follow this link to install it', 'woocommerce-eu-vat-compliance').'</a>';
				echo '<a href="https://github.com/yellowtree/wp-geoip-detect/releases">'.__('Follow this link to get it', 'woocommerce-eu-vat-compliance').'</a>';
			} elseif (file_exists(WP_PLUGIN_DIR.'/geoip-detect/geoip-detect.php')) {
				echo '<a href="'.esc_url(wp_nonce_url(self_admin_url('plugins.php?action=activate&plugin=geoip-detect/geoip-detect.php'), 'activate-plugin_geoip-detect/geoip-detect.php')).'">'.__('Follow this link to activate it.', 'woocommerce-eu-vat-compliance').'</a>';
			}
		}
		echo '</p></div>';
	}
