...
//		add_action('woocommerce_admin_order_data_after_shipping_address', array($this, 'woocommerce_admin_order_data_after_shipping_address'));
	...
	public function woocommerce_checkout_update_order_meta($order_or_id) {

		// Record the information about the customer's location in the order meta
		$compliance = WooCommerce_EU_VAT_Compliance();
// 		if (version_compare(WC_VERSION, '2.7', '>=') && !is_a($order_or_id, 'WC_Order')) return;
		$order = is_a($order_or_id, 'WC_Order') ? $order_or_id : $compliance->get_order($order_or_id);
		...
		$compliance->wc_compat->update_meta_data($order, 'vat_compliance_country_info', apply_filters('wc_eu_vat_compliance_meta_country_info', $country_info));
	...
	public function record_conversion_rates_and_other_meta($order) {

		$compliance = WooCommerce_EU_VAT_Compliance();

		// Record order number; see: http://docs.woothemes.com/document/sequential-order-numbers/
		$compliance->wc_compat->update_meta_data($order, 'order_time_order_number', $order->get_order_number());
		...
		$compliance->wc_compat->update_meta_data($order, 'wceuvat_conversion_rates', $conversion_rates);
	}
