...
abstract class WC_EU_VAT_Compliance_Rate_Provider_base_generic {
	...
	public function settings_fields() {
		$info = $this->info();
		return sprintf(__('Using this rates provider requires no configuration. ', 'woocommerce-eu-vat-compliance'), $info['url']);
	}
	...
	protected function wp_remote_get($url) {
		...
		$http_options = array(
			'timeout' => 15,
			'user-agent' => 'WooCommerce EU VAT Compliance/'.WooCommerce_EU_VAT_Compliance()->get_version().' WooCommerce/'.WC_VERSION.' PHP/'.PHP_VERSION
		);
