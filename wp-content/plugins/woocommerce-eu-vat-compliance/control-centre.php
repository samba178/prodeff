...
// TODO: Link to more documentation, if/when written
...
GeoIP is not really a setting. We need a separate panel for checking that everything is working.
		...
		echo '<h3>'.__('GeoIP configuration', 'woocommerce-eu-vat-compliance').'</h3><div>';
		...
		if (function_exists('geoip_detect_get_info_from_ip') && function_exists('geoip_detect_get_database_upload_filename')) {
			...
			echo '<h3>'.__('GeoIP location - database state and testing', 'woocommerce-eu-vat-compliance').'</h3><p><a href="'.$geoip_settings_link.'">'.__('Follow this link.', 'woocommerce-eu-vat-compliance').'</a></p>';
		}
