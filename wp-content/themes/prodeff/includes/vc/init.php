 ...
 *
 */
define( 'DHVC_ADD_ITEM_TITLE', __( "Add Item", 'luxury-wp' ) );
/**
 *
 */
define( 'DHVC_ITEM_TITLE', __( "Item", 'luxury-wp' ) );
/**
 *
 */
define( 'DHVC_MOVE_TITLE', __( 'Move', 'luxury-wp' ) );
/**
 *
 */
...
class DHVC_Init {
	...
	public function add_params() {
		...
		vc_update_shortcode_param(
			...
			array(
				...
				'param_name' => 'style',
				'value' => array_merge(getVcShared( 'separator styles' ),array( __( 'Inherit from Theme', 'luxury-wp' ) => 'inherit_theme' ) ),
		...
		if(!empty($vc_icon_lib_params) && isset($vc_icon_lib_params['value'])){
			...
			$vc_icon_lib[__( 'Elegant', 'luxury-wp' )] = 'elegant';
			vc_update_shortcode_param(
				'vc_icon',
				array(
					'weight' => 9999,
		...
		vc_add_param("vc_row", array(
				"type" => "dropdown",
				"group" => __( 'Row Type', 'luxury-wp' ),
				"class" => "",
				"heading" => "Type",
				'std' => 'full_width',
				"param_name" => "wrap_type",
				"value" => array(
					__( "Full Width", 'luxury-wp' ) => "full_width",
					__( "In Container", 'luxury-wp' ) => "in_container" ) ) );
	...
	public function product_attribute_filter_param( $settings, $value ) {
		...
		$output .= '<select id= "' . $settings['param_name'] . '" multiple="multiple"  data-placeholder="' .
			 __( 'Select Attibute Filter', 'luxury-wp' ) .
	...
	public function product_brand_param( $settings, $value ) {
		...
		$output .= '<select id= "' . $settings['param_name'] . '" multiple="multiple" data-placeholder="' .
			 __( 'Select brands', 'luxury-wp' ) . '" class="dh-chosen-multiple-select dh-chosen-select-nostd ' .
			 $settings['param_name'] . ' ' . $settings['type'] . '">';
		if ( ! empty( $brands ) ) {
	...
	public function product_lookbook_param( $settings, $value ) {
		...
		$output .= '<select id= "' . $settings['param_name'] . '" multiple="multiple" data-placeholder="' .
			 __( 'Select lookbooks', 'luxury-wp' ) . '" class="dh-chosen-multiple-select dh-chosen-select-nostd ' .
			 $settings['param_name'] . ' ' . $settings['type'] . '">';
		if ( ! empty( $lookbooks ) ) {
	...
	public function product_category_param( $settings, $value ) {
		...
		$output .= '<select id= "' . $settings['param_name'] . '" multiple="multiple" data-placeholder="' .
			 __( 'Select categories', 'luxury-wp' ) . '" class="dh-chosen-multiple-select dh-chosen-select-nostd ' .
			 $settings['param_name'] . ' ' . $settings['type'] . '">';
		if ( ! empty( $categories ) ) {
	...
	public function products_ajax_param( $settings, $value ) {
		...
		$output = '<select data-placeholder="' . __( 'Search for a product...', 'luxury-wp' ) . '" id= "' .
		...
		if ( ! empty( $product_ids ) ) {
			foreach ( $product_ids as $product_id ) {
				...
				$product_name = sprintf( __( '%s &ndash; %s', 'luxury-wp' ), $identifier, $product->get_title() );
	...
	public function productLookbooksAutocompleteSuggester($query, $slug = false){
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				( ( strlen( $value['name'] ) > 0 ) ? ' - ' . __( 'Name', 'luxury-wp' ) . ': ' . $value['name'] : '' ) .
				( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . __( 'Slug', 'luxury-wp' ) . ': ' . $value['slug'] : '' );
				$result[] = $data;
			}
		}
	...
	protected function _productTermOutput( $term,$use_slug = true) {
		...
		$term_slug_display = '';
		if ( ! empty( $term_slug ) ) {
			$term_slug_display = ' - ' . __( 'Sku', 'luxury-wp' ) . ': ' . $term_slug;
		}
	
		$term_title_display = '';
		if ( ! empty( $term_title ) ) {
			$term_title_display = ' - ' . __( 'Title', 'luxury-wp' ) . ': ' . $term_title;
		}
	
		$term_id_display = __( 'Id', 'luxury-wp' ) . ': ' . $term_id;
	...
	public function enqueue_scripts() {
		...
		$pricing_table_feature_tmpl .= '<tr><td><textarea id="content"></textarea></td><td align="left" style="padding:5px;"><a href="#" class="pricing-table-feature-remove" onclick="return pricing_table_feature_remove(this);"  title="' .esc_attr__( 'Remove', 'luxury-wp' ) . '">-</a></td></tr>';
		...
		$dhvcL10n = array( 
			'pricing_table_max_item_msg' => esc_attr__( 'Pricing Table element only support display 5 item', 'luxury-wp' ), 
