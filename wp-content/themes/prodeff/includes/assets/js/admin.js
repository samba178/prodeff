;(function($) {
	$(document).ready(function(){
		...
		$('<div id="dh_tooltip_form">'+dhAdminL10n.tooltip_form+'</div>').appendTo('body').hide();
		var dh_tooltip_form = $('.dh-tooltip-form');
		dh_tooltip_form.find('#type').on('change',function(){
			dh_tooltip_form.find('input#title').closest('div').hide();
			console.log(dh_tooltip_form);
			if($(this).val() == 'popover'){
				dh_tooltip_form.find('input#title').closest('div').show();
			}
		});
		dh_tooltip_form.find("#dh-tooltip-cancel > a").click(function(e){
			e.stopPropagation();
			e.preventDefault();
			tb_remove();
		});
		dh_tooltip_form.find('#dh-tooltip-update > input[type="button"]').click(function(e){
			e.stopPropagation();
			e.preventDefault();
			var shortcode = '[dh_tooltip ';
			$('.dh-tooltip-form input,.dh-tooltip-form select').each(function(){
			...
			shortcode +=']';
			if(dh_tooltip_form.find("#content").val() != ''){
				shortcode += dh_tooltip_form.find('[data-id="content"]').val();
			}
			shortcode +='[/dh_tooltip]';
