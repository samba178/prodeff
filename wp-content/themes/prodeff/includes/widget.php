...
class DH_Social_Widget extends DH_Widget {
	public function __construct(){
		...
		$this->widget_description = __( "Display Social Icon.", 'luxury-wp' );
		$this->widget_id          = 'DH_Social_Widget';
		$this->widget_name        = __( 'Social', 'luxury-wp' );
		
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'	=>'',
				'label' => __( 'Title', 'luxury-wp' )
			),
			'social' => array(
					'type'  => 'select',
					'std'   => '',
					'multiple'=>true,
					'label'=>__('Social','luxury-wp'),
					'desc' => __( 'Select socials', 'luxury-wp' ),
					'options' => array(
						'facebook'=>'Facebook',
						'twitter'=>'Twitter',
			...
			'style' => array(
				'type'  => 'select',
				'std'   => '',
				'label' => __( 'Style', 'luxury-wp' ),
				'options' => array(
					'none' =>  __('None', 'luxury-wp' ),
					'square' =>  __('Square', 'luxury-wp' ),
					'round' =>  __('Round', 'luxury-wp' ),
...
class DH_Tweets extends WP_Widget {
	public function __construct() {
		parent::__construct (
			'dh_tweets', 		// Base ID
			'Recent Tweets', 		// Name
			array ('classname'=>'tweets-widget','description' => __ ( 'Display recent tweets', 'luxury-wp' ) )
		);
	}
	...
	public function widget($args, $instance) {
		...
		if(empty($instance['consumerkey']) || empty($instance['consumersecret']) || empty($instance['accesstoken']) || empty($instance['accesstokensecret']) || empty($instance['cachetime']) || empty($instance['username'])){
			echo '<strong>'.__('Please fill all widget settings!' , 'luxury-wp').'</strong>' . $after_widget;
			return;
		}
		...
		if($diff >= $crt || empty($dh_widget_recent_tweets_cache_time)){
			...
			if(!require_once(DHINC_DIR . '/lib/twitteroauth.php')){
				echo '<strong>'.__('Couldn\'t find twitteroauth.php!','luxury-wp').'</strong>' . $after_widget;
				return;
			}
			...
			if(!empty($tweets->errors)){
				if($tweets->errors[0]->message == 'Invalid or expired token'){
					echo '<strong>'.$tweets->errors[0]->message.'!</strong><br/>'.__('You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', 'luxury-wp' ) . $after_widget;
				}else{
	...
	protected function _relative_time($a=''){
		...
		if(is_numeric($d) && $d > 0) {
			...
			//if less then minute
			if($d < $minute) return sprintf(__("%s seconds ago",'luxury-wp'),floor($d));
			//if less then 2 minutes
			if($d < $minute * 2) return __("about 1 minute ago",'luxury-wp');
			//if less then hour
			if($d < $hour) return sprintf(__('%s minutes ago','luxury-wp'), floor($d / $minute));
			//if less then 2 hours
			if($d < $hour * 2) return __("about 1 hour ago",'luxury-wp');
			//if less then day
			if($d < $day) return sprintf(__("%s hours ago", 'luxury-wp'),floor($d / $hour));
			//if more then day, but less then 2 days
			if($d > $day && $d < $day * 2) return __("yesterday",'luxury-wp');
			//if less then year
			if($d < $day * 365) return sprintf(__('%s days ago','luxury-wp'),floor($d / $day));
			//else return more than a year
			return __("over a year ago",'luxury-wp');
		}
	}
	...
	public function form($instance) {
		...
		<p>
			<label>' . __ ( 'Consumer Secret' , 'luxury-wp' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token' , 'luxury-wp' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token Secret' , 'luxury-wp' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Cache Tweets in every' , 'luxury-wp' ) . ':</label>
			<input type="text" name="' . $this->get_field_name ( 'cachetime' ) . '" id="' . $this->get_field_id ( 'cachetime' ) . '" value="' . esc_attr ( $instance ['cachetime'] ) . '" class="small-text" />'.__('hours','luxury-wp').'
		</p>
		<p>
			<label>' . __ ( 'Twitter Username' , 'luxury-wp' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Tweets to display' , 'luxury-wp' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Exclude replies', 'luxury-wp' ) . ':</label>
...
class DH_Post_Thumbnail_Widget extends DH_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( "Widget post with thumbnail.", 'luxury-wp' );
		$this->widget_id          = 'dh_widget_post_thumbnail';
		$this->widget_name        = __( 'Post Thumbnail', 'luxury-wp' );
		$this->cached = false;
		...
		$this->settings           = array(
				...
				'posts_per_page' => array(
					...
					'max'   => '',
					'std'   => 5,
					'label' => __( 'Number of posts to show', 'luxury-wp' )
				),
				'orderby' => array(
					'type'  => 'select',
					'std'   => 'date',
					'label' => __( 'Order by', 'luxury-wp' ),
					'options' => array(
							'latest'   => __( 'Latest', 'luxury-wp' ),
							'comment'  => __( 'Most Commented', 'luxury-wp' ),
					)
				),
				'categories' => array(
						'type'  => 'select',
						'std'   => '',
						'multiple'=>true,
						'label'=>__('Categories','luxury-wp'),
						'desc' => __( 'Select a category or leave blank for all', 'luxury-wp' ),
						'options' => $categories_options,
				),
				'hide_date' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => __( 'Hide date in post meta info', 'luxury-wp' )
				),
				'hide_comment' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => __( 'Hide comment in post meta info', 'luxury-wp' )
				),
		);
		parent::__construct();
	}
			...
			if ( $title )
				...
				while ($r->have_posts()): $r->the_post();global $post;
					...
					echo '<div class="posts-thumbnail-content">';
						...
						if(!$hide_date)
							echo '<time datetime="'.get_the_date('c').'">'.get_the_date('M d, Y').'</time>';
						...
						if(!$hide_comment){
							...
							if ( $number > 1 ) {
								$output = str_replace( '%', number_format_i18n( $number ), ( false === false ) ? __( '% Comments','luxury-wp' ) : false );
							} elseif ( $number == 0 ) {
								$output = ( false === false ) ? __( '0 Comments','luxury-wp' ) : false;
							} else { // must be one
								$output = ( false === false ) ? __( '1 Comment' ,'luxury-wp') : false;
							}
...
class DH_Mailchimp_Widget extends DH_Widget {
	public function __construct(){
		...
		$this->widget_description = __( "Widget Mailchimp Subscribe.", 'luxury-wp' );
		$this->widget_id          = 'dh_widget_mailchimp';
		$this->widget_name        = __( 'Mailchimp Subscribe', 'luxury-wp' );
		$this->cached = false;
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
...
class DH_Woocommerce_Swatches_Widget extends DH_Widget{
	public function __construct(){
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget when viewing product categories width Swatches and Photo .', 'luxury-wp' );
		$this->widget_id          = 'dh_widget_swatches';
		$this->widget_name        = __( 'DH Woocommerce Swatches Layered Nav', 'luxury-wp' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'luxury-wp' ),
				'label' => __( 'Title', 'luxury-wp' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'luxury-wp' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'luxury-wp' ),
				'options' => array(
					'and' => __( 'AND', 'luxury-wp' ),
					'or'  => __( 'OR', 'luxury-wp' )
				)
			),
		);
		...
		if ( 0 < count( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				...
				if ( isset( $_chosen_attributes2[ $taxonomy ] ) && is_array( $_chosen_attributes2[ $taxonomy ]['terms'] ) && (in_array( $term->slug, $_chosen_attributes2[ $taxonomy ]['terms'] ) || in_array( $term->term_id, $_chosen_attributes2[ $taxonomy ]['terms'] ) ) ) {
					...
					$class = 'class="chosen"';
					$filter_title = __('Remove Filter','luxury-wp');
...
class DH_Woocommerce_Product_Sorting extends DH_Widget{
	public function __construct() {
		...
		$this->widget_description = __( 'Display WooCommerce Product sorting list.', 'luxury-wp' );
		$this->widget_id          = 'dh_woocommerce_product_sorting';
		$this->widget_name        = __( 'DH WooCommerce Product Sorting', 'luxury-wp' );
	...
	public function widget( $args, $instance ) {
		...
		if ( 1 != $wp_query->found_posts || woocommerce_products_will_display() ) {
			...
			$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
				'menu_order' => __( 'Default sorting', 'luxury-wp' ),
				'popularity' => __( 'Sort by popularity', 'luxury-wp' ),
				'rating'     => __( 'Sort by average rating', 'luxury-wp' ),
				'date'       => __( 'Sort by newness', 'luxury-wp' ),
				'price'      => __( 'Sort by price: low to high', 'luxury-wp' ),
				'price-desc' => __( 'Sort by price: high to low', 'luxury-wp' )
			) );
...
class DH_Woocommerce_Price_Filter extends DH_Widget{
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'luxury-wp' );
		$this->widget_id          = 'dh_woocommerce_price_filter';
		$this->widget_name        = __( 'DH WooCommerce Price Filter', 'luxury-wp' );
	...
	public function init_settings() {
		$this->settings = array(
			'title' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => __( 'Price range size', 'luxury-wp' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => __( 'Max price ranges', 'luxury-wp' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __( 'Hide empty price ranges', 'luxury-wp' )
			)
		);
	}
...
// 		}
		...
		if(apply_filters('dh_woocommerce_price_filter_show_all', false)){
			if ( strlen( $min_price ) > 0 ) {
				echo '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'luxury-wp' ) . '</a></li>';
			} else {
				$url = remove_query_arg( array( 'min_price','max_price'), $link );
				echo '<li class="chosen"><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'luxury-wp' ) . '</a></li>';
			}
		}
		$count = 0;
