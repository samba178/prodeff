...
function dh_is_post_grid_first_full($layout,$is_shortcode=false,$grid_first=false){
	if($is_shortcode){
		if(!empty($grid_first) && $layout == 'grid')
			return true;
	}
	if(is_home() || is_front_page()){
		if(dh_get_theme_option('blogs-main-grid-first',0) && $layout == 'grid'){
			return true;
		}
	}else{
		if(dh_get_theme_option('blogs-grid-first',0) && $layout == 'grid'){
			return true;
		}
	}
	return false;
...
function dh_is_header_offcanvas_always_show(){
	if(dh_get_theme_option('header-style','classic') == 'toggle-offcanvas' && dh_get_theme_option('header-offcanvas','click') == 'always')
		return true;
	return false;
}

function dh_logo_tag(){
	$tag = 'div';
	if(is_home() || is_front_page())
		$tag = 'h1';
	echo apply_filters('dh_logo_tag', $tag);
}


function dh_offcanvas_class($class = ''){
	$classes = array();
	$header_style = dh_get_theme_option('header-style','classic');
	if($header_style == 'toggle-offcanvas'){
		$classes[] = 'header-offcanvas';
		if(dh_get_theme_option('header-offcanvas','click') == 'always')
			$classes[] = 'header-offcanvas-always-show';
	}elseif ($header_style == 'overlay'){
		$classes[] = 'header-offcanvas navbar-offcanvas-overlay';
	}elseif(dh_get_theme_option('show-navbar-offcanvas',1)){
		$classes[] = 'header-offcanvas navbar-offcanvas';
	}
	...
	$classes = apply_filters( 'dh_offcanvas_class', $classes, $class );
	
	echo 'class="' . join( ' ', $classes ) . '"';
}


function dh_currency_dropdown($label = true,$type = 'dropdown',$echo = true){
	if(!apply_filters('dh_use_currency_dropdown', false))
		return '';
	
	$currencies = apply_filters('dh_currency', array(
		'USD'=>'US Dollars (&#36;)',
		'EUR' =>'Euros (&euro;)',
	));
	$currency_active = apply_filters('dh_currency_active','USD');
	ob_start();
	?>
	<div class="currencies-switcher">
		<?php if($label) echo '<label>'.esc_html('Currency','luxury-wp').'</label>';?>
		<?php if($type == 'select'){?>
		<?php 
		echo '<div class="form-flat-select"><select>';
		foreach( $currencies as $key=>$label ){
			echo '<option'.($key == $currency_active ? ' selected="selected" ':'').' value="'. $key.'">'.esc_attr($label).'</option>';
		}
		echo '</select><i class="fa fa-caret-down"></i></div>';
		?>
		<?php }else{?>
		<?php 
		echo '<a class="active" href="'. $currency_active .'" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">';
		echo esc_attr(@$currencies[$currency_active]);
		echo '</a>';
		if( count( $currencies ) ){
			echo '<ul class="dropdown-menu">';
			foreach( $currencies as $key=>$label ){
				echo '<li><a href="#">'.esc_attr($label).'</a></li>';
			}
			echo '</ul>';
		}
		?>
	...
	<?php
	$output = apply_filters('dh_currency_dropdown', ob_get_clean());
	if($echo)
		echo $output;
	else
		return $output;
...
function dh_icl_languages_dropdown($label = true,$type = 'dropdown',$echo = true){
	ob_start();
	?>
	...
	<div class="language-switcher">
		...
		if( is_array( $languages ) ){
			...
			if($type == 'select'){
				...
				if($label){
					echo '<label>'.__('Languages','luxury-wp').'</label>';
				}
				if( count( $languages ) ){
					echo '<div class="form-flat-select"><select class="wpml-lang-dropdown" onchange="window.location.href=this.value">';
					foreach( $languages as $lang ){
						echo '<option'.($active_lang['language_code'] == $lang['language_code'] ? ' selected="selected" ':'').' value="'. $lang['url'] .'">'.esc_attr($lang['translated_name']).'</option>';
					}
					echo '</select><i class="fa fa-caret-down"></i></div>';
				}
				...
				echo '<img src="'. $active_lang['country_flag_url'] .'" alt="'. $active_lang['translated_name'] .'"/> '.esc_attr($active_lang['translated_name']);
				echo '</a>';
					
				if( count( $languages ) ){
					echo '<ul class="wpml-lang-dropdown dropdown-menu">';
					foreach( $languages as $lang ){
						echo '<li><a href="'. $lang['url'] .'"><img src="'. $lang['country_flag_url'] .'" alt="'. esc_attr($lang['translated_name']) .'"/> '.esc_attr($lang['translated_name']).'</a></li>';
					}
					echo '</ul>';
				}
	...
	<?php } ?>
	<?php 
	$output = apply_filters('dh_icl_languages_dropdown', ob_get_clean());
	if($echo)
		echo $output;
	else
		return $output;
...
function dh_instagram($username,$images_number=12,$refresh_hour){
	if (false === ($instagram = get_transient('instagram-'.sanitize_title_with_dashes($username.$images_number)))) {
	
		$remote = wp_remote_get('http://instagram.com/'.trim($username),array( 'decompress' => false ));
	
		if ( is_wp_error( $remote ) )
			return new WP_Error( 'site_down',__( 'Unable to communicate with Instagram.', 'luxury-wp' ));
	
		if ( 200 != wp_remote_retrieve_response_code( $remote ) )
			return new WP_Error( 'invalid_response',__( 'Instagram did not return a 200.', 'luxury-wp' ));
		...
		if ( !$insta_array )
			return new WP_Error( 'bad_json', __( 'Instagram has returned invalid data.', 'luxury-wp' ) );
		...
		switch ( $type ) {
			...
			default:
				foreach ( $images as $image ) {
					...
					$instagram[] = array(
						'description'   => __( 'Instagram Image', 'luxury-wp' ),
...
function dh_get_theme_option($option,$default = null){
	...
	if(is_page() || (defined('WOOCOMMERCE_VERSION') && is_woocommerce())){
		if($option == 'header-style'){
			...
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'modern-background'){
			$page_value = dh_get_post_meta('modern-background');
			if(!is_null($page_value) && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
	...
	if(isset($dh_theme_options[$option]) &&  $dh_theme_options[$option] !== null && $dh_theme_options[$option] !== false){
		$value = $dh_theme_options[$option];	
		return apply_filters('dh_get_theme_option', $value, $option);
	}else{
		return apply_filters('dh_get_theme_option', $default, $option);
	}
}
...
function dh_get_main_col_class($layout='',$body = false,$priority = 10){
	...
	if($body){
		if(is_page_template('page-no-sidebar.php') || is_page_template('page-full-width.php'))
			$layout = 'fullwidth';
		elseif (is_page_template('page-left-sliderbar.php'))
			$layout = 'left-sidebar';
		elseif (is_page_template('page-right-sliderbar.php'))
			$layout = 'right-sidebar';
		elseif(empty($layout))
			$layout = 'fullwidth';
		$layout = apply_filters('dh_get_layout_class', $layout);
...
function dh_container_class(){
	...
	if($main_layout == 'wide' || $main_layout == 'padding'){
...
function dh_enqueue_google_font(){
	...
	if(!empty($google_fonts)){
		foreach ($google_fonts as $font=>$google_font){
			wp_enqueue_style( 'dh-'.sanitize_title($font), "//fonts.googleapis.com/css?family=$font:".implode(',', array_unique( (array) $google_font['style']))."&subset=".implode(',', array_unique((array)$google_font['subset'])),false);
		}
	}
}
...
function dh_get_post_meta($meta = '',$post_id='',$default=null){
	...
	if(defined('WOOCOMMERCE_VERSION') && !in_the_loop()){
	...
	return apply_filters('dh_get_post_meta', $default, $meta, $post_id);;
}
...
<div class="related-post">
	<div class="related-post-title">
		<h3><span><?php echo __("Related News",'luxury-wp')?></span></h3>
	</div>
	<div class="row related-post-items">
		<?php while ($related->have_posts()): $related->the_post();global $post;?>
			<div class="related-post-item col-sm-6">
				...
				<h4 class="post-title" itemprop="name"><a href="<?php the_permalink();?>"><?php the_title();?></a></h4>
				...
				<div class="readmore-link">
					<a href="<?php the_permalink()?>"><?php _e("Read More", 'luxury-wp');?></a>
				</div>
			</div>
		<?php endwhile;?>
	</div>
					...
					<header class="comment-header">
						<div class="comment-info">
							...
							<span class="ispostauthor">
								<?php _e('Author', 'luxury-wp'); ?>
							</span>
							<?php endif; ?>
							<span class="comment-meta">
								<time datetime="<?php echo get_comment_time('c'); ?>">
									<?php echo sprintf(__('%1$s', 'luxury-wp') , get_comment_date('M d, Y')); ?>
								</time>
								<span class="comment-edit">
									<?php edit_comment_link('' . __('Edit', 'luxury-wp')); ?>
								</span>
							</span>
						</div>
						<?php if ('0' == $comment->comment_approved): ?>
							<p class="comment-pending"><?php _e('Your comment is awaiting moderation.', 'luxury-wp'); ?></p>
						<?php endif; ?>
					</header>
...
function dh_get_search_form(){
	...
	$search_form = '<form method="GET" class="searchform'.($ajax ?' search-ajax':'').'" action="'.esc_url( home_url( '/' ) ).'" role="form">
					...
					<input type="hidden" name="post_type" value="'.apply_filters('dh_ajax_search_form_post_type', 'any').'" />
				</form>';
	if($ajax)
...
function dh_comment_form( $args = array(), $post_id = null ) {
	...
	<div id="respond-wrap">
		<?php 
			...
			$fields =  array(
				'author' => '<div class="row"><p class="comment-form-author col-sm-12">' .'<input placeholder="'.__('Name *','luxury-wp').'" id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
				'email' => '<p class="comment-form-email col-sm-12">'.'<input id="email" placeholder="'.__('Email *','luxury-wp').'" name="email" type="text" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
				'url' => '<p class="comment-form-url col-sm-12"><input id="url"  placeholder="'.__('Website','luxury-wp').'"  name="url" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p></div>'
			);
			$comments_args = array(
					'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
					'logged_in_as'		   => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'luxury-wp' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
					'title_reply'          => sprintf('<span>%s</span>',__( 'Leave a reply', 'luxury-wp' )),
					'title_reply_to'       => sprintf('<span>%s</span>',__( 'Leave a reply to %s', 'luxury-wp' )),
					'cancel_reply_link'    => __( 'Click here to cancel the reply', 'luxury-wp' ),
					'label_submit'         => __( 'Submit', 'luxury-wp' ),
					'comment_field'		   => '<p class="comment-form-comment"><textarea placeholder="'.__('Comment','luxury-wp').'"  class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
					'must_log_in'		   => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'luxury-wp' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
			);
...
function dh_nth_word($text, $nth = 1, $echo = true,$is_typed = false,$typed_color = ''){
	...
	if ( 'characters' == _x( 'words', 'word count: words or characters?','luxury-wp') && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
...
function dh_post_nav() {
	...
	<nav class="post-navigation" role="navigation">
		<div class="row">
			<?php $prev_link = get_previous_post_link( '%link', _x( '%title', 'Previous post link', 'luxury-wp' ) ); ?>
			<div class="col-sm-6">
			<?php if($prev_link):?>
				<div class="prev-post">
					<span>
			...
			<?php $next_link = get_next_post_link( '%link', _x( '%title', 'Next post link', 'luxury-wp' ) ); ?>
			<div class="col-sm-6">
...
function dh_page_title($echo = true){
	...
	elseif (is_day())
	{
		$title = __('Archive for date:','luxury-wp')." ".get_the_time('F jS, Y');
	}
	elseif (is_month())
	{
		$title = __('Archive for month:','luxury-wp')." ".get_the_time('F, Y');
	}
	elseif (is_year())
	{
		$title = __('Archive for year:','luxury-wp')." ".get_the_time('Y');
	}
	elseif (is_search())
	{
		global $wp_query;
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$title =  $wp_query->found_posts ." ". __('search results for','luxury-wp').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
			else
			{
				$title =  $wp_query->found_posts ." ". __('search result for','luxury-wp').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
		}
		else
		{
			if(!empty($_GET['s']))
			{
				$title = __('Search results for','luxury-wp').' &ldquo;'.esc_attr( get_search_query() ).'&ldquo;';
			}
			else
			{
				$title = __('To search the site please enter a valid term','luxury-wp');
			}
		}
	...
	elseif(is_tax())
	{
	    ...
	    $title = __('404 - Page not found','luxury-wp');
	}
	else {
		$title = get_the_title();
	}
	...
	{
		$title .= " (".__('Page','luxury-wp')." ".$_GET['paged'].")";
	}
...
// 		}
	...
	if(is_post_type_archive( 'portfolio' )){
		$title = esc_html(dh_get_theme_option('portfolio-archive-title',__('My Portfolio','luxury-wp')));
	}
	if($echo)
...
function dh_post_featured($post_id='',$post_format='',$is_shortcode = false,$is_related = false,$entry_featured_class = '',$layout = '',$is_thumb_square=false){
	...
	$thumb_size = !is_singular() || $is_shortcode || $is_related ? 'dh-thumbnail' : 'post-thumbnail';

	if($layout == 'masonry'){
		$thumb_size = 'dh-full';
	}
	if($layout == 'zigzag' || $is_thumb_square){
	...
	if($is_related){
		...
		if($post_format == 'gallery'){
					...
					<div class="caroufredsel-wrap">
						<ul class="caroufredsel-items">
							<?php foreach ($gallery_ids as $id):?>
								...
								?>
								<li class="caroufredsel-item">
									<a href="<?php echo get_the_permalink() ?>" title="<?php echo get_the_title($id)?>">
										<?php echo wp_get_attachment_image($id,$thumb_size);?>
									</a>
								</li>
								<?php endif;?>
			...
			if(is_single() && !$is_shortcode){
				...
				echo '<a href="'.get_the_permalink().'" title="'.esc_attr(get_the_title(get_post_thumbnail_id($post_id))).'">'.$thumb.' <i class="elegant_arrow_triangle-right_alt2"></i></a>';
				echo '</div>';
			}
			...
			if (has_post_thumbnail()){
				...
				$thumb = '<img src="'.dh_placeholder_img_src().'">';
				echo '<div class="entry-featured'.$featured_class.' '.$entry_featured_class.'">';
				if(!is_singular() || $is_shortcode){
					echo '<a href="'.get_the_permalink().'" title="'.esc_attr(get_the_title(get_post_thumbnail_id($post_id))).'">'.$thumb.'</a>';
				}else{
...
function dh_post_meta($show_date=true,$show_comment = true,$show_category= true,$show_author = true,$echo = true,$meta_separator= ', ',$date_format = null,$icon = false) {
	...
	if($show_date){
		...
		if($icon)
			$date_html .= '<i class="fa fa-calendar"></i>';
		$date_html .= sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><time itemprop="dateCreated" datetime="%3$s">%4$s</time></a>',
	...
	if($show_author){
		...
		if($icon)
			$author_html .= '<i class="fa fa-user"></i>';//__('By', 'luxury-wp');
		$author = sprintf(
			'<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'nicename' ) ) ),
			esc_attr( sprintf( __( 'Posts by: %s', 'luxury-wp'), get_the_author() ) ),
			get_the_author()
		);
		$author_html .= sprintf(__('%1$s', 'luxury-wp'),$author);
		$author_html .= '</span>';
		$html[] = $author_html;
	}
	// Categories
	...
	if($show_category){
		...
		$categories_html .= sprintf(__('%1$s','luxury-wp'),get_the_category_list(', '));
	...
	if (comments_open()) {
		...
		if (get_comments_number() == 0) {
			$comment_title = sprintf(__('Leave a comment on: &ldquo;%s&rdquo;', 'luxury-wp') , get_the_title());
			$comment_number = '0';
			$comment_text = __('Comment','luxury-wp');
		} else if (get_comments_number() == 1) {
			$comment_title = sprintf(__('View a comment on: &ldquo;%s&rdquo;', 'luxury-wp') , get_the_title());
			$comment_number = '1';
			$comment_text = __('Comment','luxury-wp');
		} else {
			$comment_title = sprintf(__('View all comments on: &ldquo;%s&rdquo;', 'luxury-wp') , get_the_title());
			$comment_number =  get_comments_number();
			$comment_text = __('Comments','luxury-wp');
		}
		...
		$comments_html.=  $comment_number .' '.$comment_text. '</a></span> ';
...
function dh_paginate_links_short($args = array(), $query = null){
	...
	$defaults = array(
		...
		'add_fragment' => '',
		'add_args'=>array(),
		'before' => '<div class="paginate"><div class="paginate_links"><span class="pagination-meta">'.sprintf(__("%d/%d", 'luxury-wp'), $paged, $max_num_pages).'</span>',
		'after' => '</div></div>',
		'echo' => true,
	);
...
function dh_paginate_next_prev($args = array(), $query = null){
	global $wp_rewrite, $wp_query;
	if ( empty($query)) {
		$query = $wp_query;
	}
	if ( 1 >= $query->max_num_pages )
		return;
	do_action( 'dh_paginate_next_prev_start' );
	?>
	<div class="paginate">
		<div class="paginate_links paginate_next_prev clearfix">
			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous pull-left"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'luxury-wp' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next pull-right"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'luxury-wp' ) ); ?></div>
			<?php endif; ?>
		</div>
	</div>
	<?php
	do_action( 'dh_paginate_next_prev_end' );
}
...
function dh_morphsearchform(){
	...
	<div class="morphsearch" id="morphsearch">
		<form class="morphsearch-form" method="get"  action="<?php echo esc_url( home_url( '/' ) ) ?>" role="form">
			<input type="search" name="s" placeholder="<?php echo esc_attr__('Search...','luxury-wp')?>" class="morphsearch-input">
		...
		<div class="morphsearch-content<?php echo (defined( 'WOOCOMMERCE_VERSION' )  ? ' has-3colum':'') ?>">
			...
			<div class="dummy-column">
				<h2><?php _e('Product','luxury-wp') ?></h2>
				<?php 
				$query_args = array(
		    		'posts_per_page' => 6,
		    		'post_status' 	 => 'publish',
			...
			<?php }?>
			<div class="dummy-column">
				<h2><?php _e('Popular','luxury-wp') ?></h2>
				<?php 
				...
				<a href="<?php the_permalink()?>" class="dummy-media-object">
					...
					<div>
						<h3><?php the_title()?></h3>
						<?php echo '<span>'.sprintf(__('%s Comment','luxury-wp'),get_comments_number()).'</span>'; ?>
					</div>
				</a>
				<?php endwhile; ?>
				<?php 
