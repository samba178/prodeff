...
if ( ! class_exists( 'DH_Walker' ) ) :
	...
	class DH_Walker extends Walker_Nav_Menu {
		...
		protected $_megamenu_status = '';

		protected $_megamenu_fullwidth = '';

		protected $_megamenu_columns = 4;

		protected $_current_columns = 0;

		protected $_megamenu_title = '';

		protected $_megamenu_sidebar = '';

		protected $_menu_icon = '';
		
		protected $_menu_badge = '';

		protected $_menu_visibility = '';
		
		protected $_number_of_root_elements = 0;	
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$this->_menu_badge = get_post_meta( $item->ID, '_dh_menu_badge', true );
			if($this->_menu_badge)
				$this->_menu_badge = wp_get_attachment_url($this->_menu_badge);
			
			$title_badge= '';
			if ( ! empty( $this->_menu_badge ) ) {
				$title_badge = '<span class="menu-item-badge"><img src="' . $this->_menu_badge . '" alt="navbadge"></span>';
			}
			
			$this->_menu_visibility = get_post_meta( $item->ID, '_dh_menu_visibility', true );

			$class_columns .= dh_visibility_class($this->_menu_visibility);
					...
					if ( strcasecmp( $item->attr_title, 'dropdown-header' ) == 0 && $depth === 1 ) {
						...
						if ( strcasecmp( $item->attr_title, 'disabled' ) == 0 ) {
							...
							if ( ! empty( $this->_menu_icon ) ){
								$item_output .= '<i class="' . $this->_menu_icon . ' navicon"></i>';
							}
							
							$item_output .= $args->link_before. $title_badge . apply_filters( 'the_title', $item->title, $item->ID ) .$args->link_after;
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $depth === 1 && $this->_megamenu_status === 'yes' ) {
				...
				if ( $depth === 2 && ! empty( $this->_megamenu_sidebar ) && is_active_sidebar( $this->_megamenu_sidebar ) && $this->_megamenu_status == "yes" ) {
					...
					if ( $depth === 0 ) {
						$item_output .= '<span class="underline">' . $args->link_before .$title_badge. apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after . '</span>';
					} else {
						...
						if ( strcasecmp( $item->attr_title, 'disabled' ) == 0 ) {
							...
							if( $depth === 0 && $args->has_children ) {
								if( $this->_megamenu_status == "yes" ) {
									...
									$class_names .= ' dh-megamenu-menu-col-'.absint($this->_megamenu_columns);
								} else {
									$class_names .= ' dh-megamenu-menu';
								}
							}
