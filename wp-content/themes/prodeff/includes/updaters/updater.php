...
class DH_Updater{
	...
	public function pre_upgrader($reply, $package, $updater){
		...
		if ( ! $res ) {
			return new WP_Error( 'no_credentials', __( "Error! Can't connect to filesystem", 'luxury-wp' ) );
		}
		...
		if ( empty($license_key) || empty($api)  || empty($username)) {
			...
			return new WP_Error( 'no_credentials', sprintf(__( 'To receive automatic updates license activation is required. Please visit <a href="%1$s" target="_blank">Settings</a> to activate %2$s.', 'luxury-wp'),$url,$this->get_theme_name() )  );
		}

		$updater->strings['downloading_package_url'] = __( 'Getting download link...', 'luxury-wp' );
		...
		if ( $json['response']['code'] == 200 ) {
			if ( isset( $result['error']) ) {
				return new WP_Error( 'no_credentials', __( 'Error! Envato API error', 'luxury-wp' ) . ( isset( $result['error'] ) ? ': ' . $result['error'] : '.' ) );
			}
			if ( ! isset( $result['download-purchase']['wordpress_theme'] ) ) {
				return new WP_Error( 'no_credentials', __( 'Error! Envato API error', 'luxury-wp' ) . ( isset( $result['error'] ) ? ': ' . $result['error'] : '.' ) );
			}
			
			$updater->strings['downloading_package'] = __( 'Downloading package...', 'luxury-wp' );
