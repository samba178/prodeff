	...
	class DH_Controller {
		...
		public function facebook_init(){
			...
			if( !isset( $_REQUEST['fb_response'] ) || !isset( $_REQUEST['fb_user'] )) {
				die(json_encode(array('error' => __('Authenication required.', 'luxury-wp'))));
			}
			$redirect = '';
			...
			if( !$fb_userid ) {
				die(json_encode(array('error' => __('Please connect your facebook account.', 'luxury-wp'))));
			}
			...
			if( !$user_ID ){
				...
				if( !$user_ID )
				{
					if ( !get_option( 'users_can_register' )) {
						die( json_encode( array( 'error' => __('Registration is not open at this time. Please come back later.', 'luxury-wp') )));
					}
					if (dh_get_theme_option('facebook_login',0) == 0) {
						die( json_encode( array( 'error' => __('Registration using Facebook is not currently allowed. Please use our Register page', 'luxury-wp') )));
					}
					extract( $fb_user );
					$display_name = $name;
					...
					if( empty( $verified ) || !$verified ) {
						die(json_encode(array('error' => __('Your facebook account is not verified. You have to verify your account before proceed login or registering on this site.', 'luxury-wp'))));
					}
			
					$user_email = $email;
					if ( empty( $user_email )) {
						die(json_encode(array('error' => __('Please click again to login with Facebook and allow the application to use your email address', 'luxury-wp'))));
					}
			
					if( empty( $name )) {
						die(json_encode(array('error' => 'empty_name', __('We didn\'t find your name. Please complete your facebook account before proceeding.', 'luxury-wp'))));
					}
			...
			die( json_encode( array( 'loggedin' => true, 'type' => $logintype, 'url' => $redirect, 'siteUrl' => home_url(), 'message' => __( 'Login successful, redirecting...','luxury-wp' ) )));
		...
		public function login(){
			...
			if ( is_wp_error( $user_signon ) ){
				...
				wp_send_json(array('loggedin'=>true, 'redirecturl' => $redirecturl, 'message'=> '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __( 'Login successful, redirecting...','luxury-wp' ) . '</span>' ));
			}
			die();
		}
		public function register(){
			if( !check_ajax_referer('dh-ajax-register-nonce', 'security', false) ) {
				$result = array(
					'success' => false,
					'message' => '<span class="error-response">'.__( 'Your session is expired or you submitted an invalid form.', 'luxury-wp' ).'</span>',
				);
				wp_send_json($result);
			}
			if(get_option( 'users_can_register' )){
				...
				if ( is_wp_error( $errors ) ) {
					...
					$result = array(
						'success'     => true,
						'message'	=> '<span class="success-response"><i class="fa fa-check-circle"></i> '.__( 'Registration complete.', 'luxury-wp' ).'</span>',
				...
				$result = array(
					'success' => false,
					'message'   =>__( 'Not allow register in site.', 'luxury-wp' ),
				);
			}
			wp_send_json($result);
		}
		...
		protected function _register_new_user($user_login, $user_email, $user_password='', $cuser_password=''){
			...
			if ( $sanitized_user_login == '' ) {
				$errors->add( 'empty_username', __( 'Please enter a username.', 'luxury-wp' ) );
			} elseif ( ! validate_username( $user_login ) ) {
				$errors->add( 'invalid_username', __( 'This username is invalid because it uses illegal characters. Please enter a valid username.', 'luxury-wp' ) );
				$sanitized_user_login = '';
			} elseif ( username_exists( $sanitized_user_login ) ) {
				$errors->add( 'username_exists', __( 'This username is already registered. Please choose another one.', 'luxury-wp' ) );
			}
			
			// Check the email address
			if ( $user_email == '' ) {
				$errors->add( 'empty_email', __( 'Please type your email address.', 'luxury-wp' ) );
			} elseif ( ! is_email( $user_email ) ) {
				$errors->add( 'invalid_email', __( 'The email address isn\'t correct.', 'luxury-wp' ) );
				$user_email = '';
			} elseif ( email_exists( $user_email ) ) {
				$errors->add( 'email_exists', __( 'This email is already registered, please choose another one.', 'luxury-wp' ) );
			}
			//Check the password
			...
			if(empty($user_password)){
				...
				if(strlen($user_password) < 6){
					$errors->add( 'minlength_password', __( 'Password must be 6 character long.', 'luxury-wp' ) );
				}elseif (empty($cuser_password)){
					$errors->add( 'not_cpassword', __( 'Not see password confirmation field.', 'luxury-wp' ) );
				}elseif ($user_password != $cuser_password){
					$errors->add( 'unequal_password', __( 'Passwords do not match.', 'luxury-wp' ) );
				}
			}
			...
			if ( ! $user_id ) {
				$errors->add( 'registerfail', __( 'Couldn\'t register you... please contact the site administrator', 'luxury-wp' ) );
				return $errors;
			}
			...
			// user email
			$subject = sprintf(__('Welcome to [%1$s]','luxury-wp'),$blogname);
			$to = $user->email;
			
			$message = __('Hi %1$s,<br/><br/>You\'ve just successfully registered an account on %2$s.<br/>Start reading now! %3$s<br/><br/>Best regards,<br/>%4$s','luxury-wp');
		...
		public function register_priv(){
			...
			wp_send_json(array('success'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','luxury-wp'),$link)));
			die();
		}
		
		public function login_priv(){
			$link = "javascript:window.location.reload();return false;";
			wp_send_json(array('loggedin'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','luxury-wp'),$link)));
			die();
		}
		...
		public function lostpassword(){
			...
			if ( isset($_POST) ) {
				...
				if ( empty( $_POST['user_login'] ) ) {
					$errors->add('empty_username', __('<strong>ERROR</strong>: Enter a username or e-mail address.','luxury-wp'));
				} else if ( strpos( $_POST['user_login'], '@' ) ) {
					$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
					if ( empty( $user_data ) )
						$errors->add('invalid_email', __('<strong>ERROR</strong>: There is no user registered with that email address.','luxury-wp'));
				} else {
				...
				if ( !$user_data ) {
					$errors->add('invalidcombo', __('<strong>ERROR</strong>: Invalid username or e-mail.','luxury-wp'));
					echo '<span class="error-response">' . $errors->get_error_message() . '</span>';
					die();
				}
				...
				*/
				...
				if ( ! $allow ) {
					echo '<span class="error-response">' . __('Password reset is not allowed for this user','luxury-wp') . '</span>';
					die();
				}
				...
				if(function_exists('get_password_reset_key')){
					...
					*/
					...
					if ( false === $key_saved ) {
						echo '<span class="error-response">' . __( 'Could not save password reset key to database.','luxury-wp') . '</span>';
						die();
					}
				}
				
				$message = __('Someone requested that the password be reset for the following account:','luxury-wp') . "\r\n\r\n";
				$message .= network_home_url( '/' ) . "\r\n\r\n";
				$message .= sprintf(__('Username: %s','luxury-wp'), $user_login) . "\r\n\r\n";
				$message .= __('If this was a mistake, just ignore this email and nothing will happen.','luxury-wp') . "\r\n\r\n";
				$message .= __('To reset your password, visit the following address:','luxury-wp') . "\r\n\r\n";
				...
				*/
				...
				$title = sprintf( __('[%s] Password Reset','luxury-wp'), $blogname );
				...
				*/
				...
				if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
					echo '<span class="error-response">' . __("Failure!", 'luxury-wp');
					echo __('The e-mail could not be sent.','luxury-wp');
					echo "</span>";
					die();
				} else {
					echo '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __("Email successfully sent!", 'luxury-wp')."</span>";
					die();
				}
			}
			die();
		...
		protected function _mailchimp_subscribe($is_ajax = false) {
			...
			if(!wp_verify_nonce( $nonce, 'mailchimp_subscribe_nonce' )){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Verify Nonce Failed','luxury-wp') . '</span>' ));
				else
					wp_die(__('Verify Nonce Failed','luxury-wp'));
			}
			if(!filter_var(@$data['email'], FILTER_VALIDATE_EMAIL)){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Please enter a valid email address','luxury-wp') . '</span>' ));
				else
					wp_die(__('Invalid Email','luxury-wp'));
			}
			...
			if($is_ajax){
				if($success == 1){
					wp_send_json(array( 'success' => true, 'message' => '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __('<strong>Well done!</strong> Subscribe to our Newsletter successful!','luxury-wp') . '</span>' ));
				}else{
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Can not subscribe our Newsletter','luxury-wp') . '</span>' ));
				}
			}else{
		...
		public function search() {
			...
			$s_post_type =  apply_filters('dh_ajax_search_form_post_type', 'any');
			$output = "";
			$args = array( 
			...
			if ( empty( $posts ) ) {
				...
				$output .= __( 'No results matched!', 'luxury-wp' );
				$output .= '</div>';
			} else {
				$post_types = array();
				...
				$output .= '</div>';
				$output .= '<div class="search-view-all"><a href="' . home_url( '?s=' . $_REQUEST['s'] ) . '&post_type='.($s_post_type).'">' .
					 __( 'View all results', 'luxury-wp' ) . '</a>';
			}
