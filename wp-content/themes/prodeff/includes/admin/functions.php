...
function dh_get_mailchimplist(){
	$options = array(__('Nothing Found...','luxury-wp'));
	if($mailchimp_api = dh_get_theme_option('mailchimp_api','')){
		...
		$lists = $api->lists();
		if ($api->errorCode){
			$options = array(__("Unable to load MailChimp lists, check your API Key.", 'luxury-wp'));
		}else{
			if ($lists['total'] == 0){
				$options = array(__("You have not created any lists at MailChimp",'luxury-wp'));
			}else{
				$options = array(__('Select a list','luxury-wp'));
		...
		if( isset($field['callback']) && !empty($field['callback']) ) {
			...
			switch ($field['type']){
				...
				case 'widgetised_sidebars':
					...
					echo '<option value="">' . __('Select a sidebar...','luxury-wp') . '</option>';
				...
				case 'gallery':
						...
						jQuery(document).ready(function($) {
							$('.dh-meta-gallery-select').on('click',function(e){
								...
								dh_meta_gallery_frame = wp.media({
									title: '<?php echo esc_js(__('Add Images to Gallery','luxury-wp'))?>',
									button: {
										text: '<?php echo esc_js(__('Add to Gallery','luxury-wp'))?>',
									},
									library: { type: 'image' },
									multiple: true
								});
											...
											dh_meta_gallery_list.append('\
												<li data-id="' + attachment.id +'">\
													<div class="thumbnail">\
														...
														<a href="#" title="<?php echo esc_js(__('Delete','luxury-wp'))?>">x</a></li>\
													</div>\
												</li>'
											);
										}
							...
							?>
								<li data-id="<?php echo esc_attr( $attachment_id ) ?>">
									<div class="thumbnail">
										...
										<a title="<?php echo esc_attr__('Delete','luxury-wp') ?>" href="#">x</a>
									</div>						
								</li>
							<?php
								endif;
					...
					echo '<input type="button" class="button button-primary dh-meta-gallery-select" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . __('Add Gallery Images','luxury-wp') . '" /> ';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dh_print_string( $field['description'] ) . '</span>';
					}
					echo '</div>';
				...
				case 'media':
					...
					$btn_text = !empty(  $field['value'] ) ? __( 'Change Media', 'luxury-wp' ) : __( 'Select Media', 'luxury-wp' );
					echo '<div  class="dh-meta-box-field ' . esc_attr( $field['id'] ) . '_field">';
					echo '<label for="' . esc_attr( $field['id'] ) . '">' . esc_html( $field['label'] ) . '</label>';
					...
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear', 'luxury-wp' ) . '" />';				
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dh_print_string( $field['description'] ) . '</span>';
					}
					echo '</div>';
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dh_<?php echo esc_attr($field['id']); ?>_media_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo esc_js(__( 'Select or Upload your Media', 'luxury-wp' )); ?>",
									button: {
								...
								dh_<?php echo esc_attr($field['id']); ?>_media_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo esc_js(__( 'Change Media', 'luxury-wp' )); ?>');
							...
							$('#<?php echo esc_attr($field['id']) ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']) ?>_upload').attr('value', '<?php echo esc_js(__( 'Select Media', 'luxury-wp' )); ?>');
				...
				case 'image':
					...
					echo '<input type="button" class="button button-primary" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . $btn_text . '" /> ';
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear Image', 'luxury-wp' ) . '" />';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dh_print_string( $field['description'] ) . '</span>';
					}
					?>
