...
if ( ! class_exists( 'DH_Woocommerce' ) ) :
	class DH_Woocommerce {
		...
		public function init() {
			...
			add_shortcode('woocommerce_cart',array(__CLASS__,'woocommerce_cart'));
		...
		public static function woocommerce_cart(){
			return is_null( WC()->cart ) ? '' : WC_Shortcodes::shortcode_wrapper( array( 'WC_Shortcode_Cart', 'output' ),array(),array(
				'class'  => 'woocommerce-cart-wrap woocommerce',
				'before' => null,
				'after'  => null
			) );
		}
		 ...
		 * @return array
		 */
		public static function coupons_enabled(){
			if(defined('YITH_YWGC_VERSION'))
				return apply_filters( 'dh_woocommerce_coupons_enabled', 'yes' === get_option( 'woocommerce_enable_coupons' ) );
			else
				return WC()->cart->coupons_enabled();
		}
		...
		public static function countdown_html(){
			...
			<?php if(isset($woocommerce_loop['show_countdown']) && $woocommerce_loop['show_countdown'] && $product->is_on_sale()){?>
				...
				<div class="countdown-item">
					...
					<div class="countdown-item-label">'.esc_html__('Day','luxury-wp').'</div>
				</div>
				<div class="countdown-item">
					<div class="countdown-item-value">%H</div>
					<div class="countdown-item-label">'.esc_html__('Hours','luxury-wp').'</div>
				</div>
				<div class="countdown-item">
					<div class="countdown-item-value">%M</div>
					<div class="countdown-item-label">'.esc_html__('Mins','luxury-wp').'</div>
				</div>
				<div class="countdown-item">
					<div class="countdown-item-value">%S</div>
					<div class="countdown-item-label">'.esc_html__('Secs','luxury-wp').'</div>
				</div>';
				?>
			...
			if ( is_singular( 'product' ) ) {
				...
				<?php if ( $wp_query->found_posts && woocommerce_products_will_display() ) {?>
					<div class="shop-toolbar">
						<div class="view-mode">
							<a class="grid-mode<?php echo ($current_view_mode == 'grid' ? ' active' :'')?>" title="<?php esc_attr_e('Grid','luxury-wp')?>" <?php echo ($grid_mode_href)?>><i class="fa fa-th"></i></a>
							<a class="list-mode<?php echo ($current_view_mode == 'list' ? ' active' :'')?>" title="<?php esc_attr_e('List','luxury-wp')?>" <?php echo ($list_mode_href) ?>><i class="fa fa-list"></i></a>
							<?php if(dh_get_theme_option('woo-shop-filter',0)):?>
							<div class="filter-toggle-button">
								<a class="filter" title="<?php esc_attr_e('Filter','luxury-wp')?>" href="#"><i class="fa fa-filter"></i> <?php esc_html_e('Filter','luxury-wp')?></a>
							</div>
						<?php endif;?>
						</div>
						<?php if(!dh_get_theme_option('woo-shop-filter',0)):?>
		...
		public function subcategory_thumbnail($category){
			$small_thumbnail_size  	= apply_filters( 'dh_subcategory_archive_thumbnail_size', 'dh-full' );
		...
		protected function _cart_items_text( $count ) {
			...
			if ( $count > 1 ) {
				$product_item_text = str_replace( '%', number_format_i18n( $count ), __( '% items', 'luxury-wp' ) );
			} elseif ( $count == 0 ) {
				$product_item_text = __( '0 items', 'luxury-wp' );
			} else {
				$product_item_text = __( '1 item', 'luxury-wp' );
			}
		...
		protected function _get_minicart_mobile() {
			...
			$cart_output = '<a href="' . $woocommerce->cart->get_cart_url() . '" title="' . __( 'View Cart', 'luxury-wp' ) . '"  class="cart-icon-mobile">'.$this->_get_minicart_icon2().' '.( ! empty( $cart_count ) ? '<span>' . $cart_count . '</span>' : '' ) . '</a>';
			return $cart_output;
		}
		...
		public function get_minicart_side(){
			echo '<div class="minicart-side"><div class="minicart-side-title"><h4>'.esc_html__('Shopping Cart','luxury-wp').'</h4></div><div class="minicart-side-content">'.$this->_get_minicart(true,true).'</div></div>';
			return;
		}
		...
		protected function _get_minicart( $content = false, $_flag = false ,$topbar=false) {
			...
			$minicart = '';
			if ( ! $content ) {
				$minicart .= '<a class="minicart-link" href="' . $cart_url . '"><span class="minicart-icon '.$cart_has_item.'">'.$this->_get_minicart_icon2() . '<span>'.$cart_count.'</span></span>'.($topbar ? __('My Cart','luxury-wp'):'').'</a>';
			...
			if( ((dh_get_theme_option('woo-minicart-style','side') == 'mini') ||  $content)){
				if (! empty( $cart_count ) ) {
					$minicart .= '<div class="minicart-header">' . $cart_count_text . ' ' . sprintf(__( 'in the <a href="%1$s">shopping cart</a>', 'luxury-wp' ),$cart_url) . '</div>';
					...
					foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) {
						...
						if ( $cart_product->exists() && $cart_item['quantity'] > 0 ) {
							...
							$minicart .= '<div class="cart-product-quantity-price">' . $cart_item['quantity'] . ' x ' .wc_price( $cart_product->get_price() ) . '</div>';
							// $minicart .= '<div class="cart-product-quantity">' . __('Quantity:', 'luxury-wp') . ' ' .
							// $cart_item['quantity'] . '</div>';
							$minicart .= '</div>';
							$minicart .= apply_filters( 'woocommerce_cart_item_remove_link',sprintf( '<a href="%s" class="remove" title="%s">&times;</a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'luxury-wp' ) ), $cart_item_key );
							$minicart .= '</div>';
						}
					}
					$minicart .= '</div></div>';
					$minicart .= '<div class="minicart-footer">';
					$minicart .= '<div class="minicart-total">' . __( 'Cart Subtotal:', 'luxury-wp' ) . ' ' . $cart_total . '</div>';
					...
					if ( version_compare( WOOCOMMERCE_VERSION, "2.1.0" ) >= 0 ) {
						...
						$minicart_action .= '<a class="viewcart-button button" href="' . esc_url( $cart_url ) . '"><span class="text">' . __( 'View Cart', 'luxury-wp' ) . '</span></a>';
						$minicart_action .= '<a class="checkout-button button" href="' . esc_url( $checkout_url ) .'"><span class="text">' . __( 'Checkout', 'luxury-wp' ) . '</span></a>';
					} else {
						$minicart_action .= '<a class="viewcart-button button" href="' . esc_url( $woocommerce->cart->get_cart_url() ) . '"><span class="text">' . __( 'View Cart', 'luxury-wp' ) . '</span></a>';
						$minicart_action .= '<a class="checkout-button button" href="' . esc_url( $woocommerce->cart->get_checkout_url() ) . '"><span class="text">' . __( 'Checkout', 'luxury-wp' ) . '</span></a>';
					}
					...
					$minicart .= '<div class="minicart-header no-items show">' . __( 'Your shopping bag is empty.', 'luxury-wp' ) . '</div>';
					$shop_page_url = "";
					if ( version_compare( WOOCOMMERCE_VERSION, "2.1.0" ) >= 0 ) {
					...
					$minicart .= '<div class="minicart-footer">';
					$minicart .= '<div class="minicart-actions clearfix">';
					$minicart .= '<a class="button no-item-button" href="' . esc_url( $shop_page_url ) . '"><span class="text">' . __( 'Go to the shop', 'luxury-wp' ) . '</span></a>';
					$minicart .= '</div>';
					$minicart .= '</div>';
				}
			}
		...
		public function template_loop_quickview() {
			...
			if(apply_filters('dh_woocommerce_quickview', true))
				echo '<div class="shop-loop-quickview"><a data-product_id ="' . $product->get_id() . '" title="' .esc_attr__( 'Quick view', 'luxury-wp' ) . '" href="' . esc_url( $product->get_permalink() ) . '"><i class="elegant_icon_zoom-in_alt"></i></a></div>';
		}
		...
		public function the_product_video(){
			...
			if(!empty($video)){
				...
				$html .= '</div>';
				echo ($html);
				echo '<a class="video-embed-action" data-container="body" data-trigger="hover" data-toggle="tooltip" title="'.esc_attr__('Watch Video','luxury').'" data-video-inline="'.esc_attr($video).'" href="#'.esc_attr($video_id).'" data-rel="magnific-popup-video"><i class="elegant_arrow_triangle-right_alt2"></i></a>';
			}
		}
		...
		protected function _formatted_name( WC_Product $product ) {
			...
			return sprintf( __( '%s &ndash; %s', 'luxury-wp' ), $identifier, $product->get_title() );
		}
	...
	<g>
		...
		public function product_tax_add_heading_fields($taxonomy){
			...
			<div class="form-field">
				<label for="product_cat_short_description"><?php _e('Short Description','luxury-wp')?></label>
				<input id="product_cat_short_description" type="text" aria-required="true" size="40" value="" name="product_cat_short_description">
			</div>
			<?php endif;?>
			<div class="form-field">
				<label><?php _e( 'Heading Background', 'luxury-wp' ); ?></label>
				...
				<div style="line-height:60px;">
					...
					<button type="submit" class=" button product_cat_heding_upload"><?php _e('Upload/Add image', 'luxury-wp'); ?></button>
					<button type="submit" class=" button product_cat_heding_remove"><?php _e('Remove image', 'luxury-wp'); ?></button>
				</div>
		...
		public function product_tax_edit_heading_fields($term, $taxonomy){
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="product_cat_heading_title"><?php _e('Heading Title','luxury-wp')?></label></th>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="product_cat_heading_sub_title"><?php _e('Heading Sub Title','luxury-wp')?></label></th>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="product_cat_heading_button_text"><?php _e('Heading Button Text','luxury-wp')?></label></th>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="product_cat_heading_button_link"><?php _e('Heading Sub Link','luxury-wp')?></label></th>
