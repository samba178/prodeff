...
+ function($) {
    ...
    DH.init = function() {
        ...
        // Fixed Footer
        var _fixed_footer = $('.footer-fixed');

        if (_fixed_footer.length) {
            var footerHeight = _fixed_footer.outerHeight(),
                _footer_space = $('.footer-space');
            ...
            imagesLoaded($('body'), function() {
                footerHeight = _fixed_footer.outerHeight();
                _footer_space.css({
                    height: footerHeight
                });
            });
    ...
    DH.ajaxSearchInit = function() {
        ...
        $(document).on('click', '.navbar-search-button', function(e) {
            e.stopPropagation();
            e.preventDefault();
            if (DH.enableAnimation() && $('.search-form-wrap').length) {
...
//        $( document ).on('click','div.woocommerce > form input[type=submit]',function() );
//        $( document ).on('change input','div.woocommerce form .shop_table.cart .cart_item :input',function(){
//        	$(this).closest('form').find( 'input[name="update_cart"]' ).prop( 'disabled', false );		
//		});
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if ($('.woo-content').data('paginate') == 'infinite_scroll') {
                        $('.woo-content').find('.infinite-scroll-wrap').infinitescroll('update', {
                            maxPage: $('.woo-content').find('ul.products').data('maxpage'),
                            path: DH.determinepath($('.woo-content').find('div.paginate a.next').attr('href')),
                            state: {
                                isDuringAjax: false,
                                isInvalidPage: false,
                                isDestroyed: false,
                        ...
                        $('.woo-content').dhLoadmore('update', {
                            navSelector: $('.woo-content').find('div.paginate'),
                            nextSelector: $('.woo-content').find('div.paginate a.next'),
                            state: {
                                currPage: 1,
                                maxPage: undefined,
                                isBeyondMaxPage: false,
