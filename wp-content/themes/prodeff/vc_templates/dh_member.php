...
if(!empty($avatar)){
	...
	if($style == 'below' && (!empty($facebook) || !empty($twitter) || !empty($google) || !empty($linkedin))){
		...
		if(!empty($facebook)){
			...
			$output .='<a href="'.esc_url($facebook).'" title="'.__('Facebook','luxury-wp').'" target="_blank"><i class="fa fa-facebook"></i></a>';
			$output .='</span>';
		}
		if(!empty($twitter)){
			$output .='<span class="twitter">';
			$output .='<a href="'.esc_url($twitter).'" title="'.__('Twitter','luxury-wp').'" target="_blank"><i class="fa fa-twitter"></i></a>';
			$output .='</span>';
		}
		if(!empty($google)){
			$output .='<span class="google">';
			$output .='<a href="'.esc_url($google).'" title="'.__('Google +','luxury-wp').'" target="_blank"><i class="fa fa-google"></i></a>';
			$output .='</span>';
		}
		if(!empty($linkedin)){
			$output .='<span class="linkedin">';
			$output .='<a href="'.esc_url($linkedin).'" title="'.__('Linked In','luxury-wp').'" target="_blank"><i class="fa fa-linkedin"></i></a>';
			$output .='</span>';
		}
		$output .='</div>';
	}
